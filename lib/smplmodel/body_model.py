...
class SMPLlayer(nn.Module):
        ...
        **kwargs
        ...
        if self.use_shape_blending:
            if self.model_type == "smplx":
                ...
                self.num_shapes = 10
                self.shapedirs = self.shapedirs[
                    :, :, : self.num_shapes + self.num_expression_coeffs
                ]
        ...
        if regressor_path is not None and use_joints:
            ...
            if self.use_pose_blending:
                j_posedirs = torch.einsum(
                    "ab, bde->ade", [X_regressor, torch.Tensor(posedirs)]
                ).numpy()
        ...
        if self.model_type == "smplh":
            ...
            for key in ["Ll", "Rr"]:
                val = to_tensor(
                    to_np(data["hands_mean" + key[1]].reshape(1, -1)), dtype=dtype
                )
                self.register_buffer("mHandsMean" + key[0], val)
                val = to_tensor(
                    to_np(data["hands_components" + key[1]][: self.num_pca_comps, :]),
                    dtype=dtype,
                )
                self.register_buffer("mHandsComponents" + key[0], val)
            self.use_pca = True
    ...
    def extend_pose(self, poses):
        ...
        if self.model_type not in ["smplh", "smplx", "mano"]:
            ...
            self.model_type == "smplh" and poses.shape[-1] == 156 and self.use_flat_mean
        ):
            return poses
        elif (
            self.model_type == "smplx" and poses.shape[-1] == 165 and self.use_flat_mean
        ):
            return poses
        ...
        poses_rh = poses[
            :, NUM_BODYJOINTS + NUM_HANDJOINTS : NUM_BODYJOINTS + NUM_HANDJOINTS * 2
        ]
        if self.use_pca:
        ...
        if self.model_type == "smplh":
            ...
            poses = torch.cat(
                [poses[:, :NUM_BODYJOINTS], poses_head, poses_lh, poses_rh], dim=1
            )
        return poses
    ...
    def convert_from_standard_smpl(
        self, poses, shapes, Rh=None, Th=None, expression=None
    ):
        if "torch" not in str(type(poses)):
            dtype, device = self.dtype, self.device
            poses = to_tensor(poses, dtype, device)

...
def batch_rigid_transform(rot_mats, joints, parents, dtype=torch.float32):
    ...
    transforms_mat = transform_mat(
        rot_mats.view(-1, 3, 3), rel_joints.contiguous().view(-1, 3, 1)
...
def batch_dqs_blending(A, W, Vs):
    ...
    ws = (
        torch.sqrt(
            torch.clamp(Rs[:, 0, 0] + Rs[:, 1, 1] + Rs[:, 2, 2] + 1.0, min=1.0e-6)
        )
        / 2.0
    )
    ...
    b0 = W.unsqueeze(-2) @ torch.cat(
        [ws[:, None], xs[:, None], ys[:, None], zs[:, None]], dim=-1
    ).reshape(
        Bnum, 1, Jnum, 4
    )  # B,V,J,4
    be = W.unsqueeze(-2) @ torch.cat(
        [vDw[:, None], vDx[:, None], vDy[:, None], vDz[:, None]], dim=-1
    ).reshape(
        Bnum, 1, Jnum, 4
    )  # B,V,J,4
    b0 = b0.reshape(-1, 4)

...
def test_one_epoch(args, net, test_loader):
    ...
    for (
        ...
        loss = F.mse_loss(torch.matmul(rotation_ab_pred.transpose(2, 1), rotation_ab),
                          identity) + F.mse_loss(translation_ab_pred, translation_ab)
        if args.cycle:
            rotation_loss = F.mse_loss(torch.matmul(rotation_ba_pred, rotation_ab_pred),
                                       identity.clone())
...
def test(args, net, test_loader, boardio, textio):
    ...
    textio.cprint("A--------->B")
    textio.cprint(
        f"EPOCH:: {-1}, Loss: {test_loss}, Cycle Loss: {test_cycle_loss}, MSE: {test_mse_ab}, RMSE: {test_rmse_ab}, MAE: {test_mae_ab}, rot_MSE: {test_r_mse_ab}, rot_RMSE: {test_r_rmse_ab}, "
        "rot_MAE: {test_r_mae_ab}, trans_MSE: {test_t_mse_ab}, trans_RMSE: {test_t_rmse_ab}, trans_MAE: {test_t_mae_ab}"
    )

    textio.cprint("B--------->A")
    textio.cprint(
        f"EPOCH:: {-1}, Loss: {test_loss}, Cycle Loss: {test_mse_ba}, MSE: {test_rmse_ba}, RMSE: {test_rmse_ba}, MAE: {test_mae_ba}, rot_MSE: {test_r_mse_ba}, rot_RMSE: {test_r_rmse_ba}, "
...
def train(args, net, train_loader, test_loader, boardio, textio):
    ...
    for epoch in range(args.epochs):
        ...
        textio.cprint("A--------->B")
        textio.cprint(
            f"EPOCH:: {-1}, Loss: {train_loss}, Cycle Loss: {train_cycle_loss}, MSE: {train_mse_ab}, RMSE: {train_rmse_ab}, MAE: {train_mae_ab}, rot_MSE: {train_r_mse_ab}, rot_RMSE: {train_r_rmse_ab}, "
            "rot_MAE: {train_r_mae_ab}, trans_MSE: {train_t_mse_ab}, trans_RMSE: {train_t_rmse_ab}, trans_MAE: {train_t_mae_ab}"
        )

        textio.cprint("B--------->A")
        textio.cprint(
            f"EPOCH:: {-1}, Loss: {train_loss}, Cycle Loss: {train_mse_ba}, MSE: {train_rmse_ba}, RMSE: {train_rmse_ba}, MAE: {train_mae_ba}, rot_MSE: {train_r_mse_ba}, rot_RMSE: {train_r_rmse_ba}, "
        ...
        textio.cprint("A--------->B")
        textio.cprint(
            f"EPOCH:: {-1}, Loss: {best_test_loss}, Cycle Loss: {best_test_cycle_loss}, MSE: {best_test_mse_ab}, RMSE: {best_test_rmse_ab}, MAE: {best_test_mae_ab}, rot_MSE: {best_test_r_mse_ab}, rot_RMSE: {best_test_r_rmse_ab}, "
            "rot_MAE: {best_test_r_mae_ab}, trans_MSE: {best_test_t_mse_ab}, trans_RMSE: {best_test_t_rmse_ab}, trans_MAE: {best_test_t_mae_ab}"
        )

        textio.cprint("B--------->A")
        textio.cprint(
            f"EPOCH:: {-1}, Loss: {best_test_loss}, Cycle Loss: {best_test_mse_ba}, MSE: {best_test_rmse_ba}, RMSE: {best_test_rmse_ba}, MAE: {best_test_mae_ba}, rot_MSE: {best_test_r_mse_ba}, rot_RMSE: {best_test_r_rmse_ba}, "
